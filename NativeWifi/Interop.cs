...
namespace NativeWifi
{
	...
	public static class Wlan
	{
		...
		public class WlanBssEntry{
			...
			internal byte[] dot11Bssid;
			
			/// <summary>
			/// Specifies whether the network is infrastructure or ad hoc.
			/// </summary>
			internal Dot11BssType dot11BssType;
			internal Dot11PhyType dot11BssPhyType;
			/// <summary>
			...
			/// </summary>
			internal uint chCenterFrequency;
			/// <summary>
			/// Contains the set of data transfer rates supported by the BSS.
			/// </summary>
			internal WlanRateSet wlanRateSet;
			/// <summary>
			/// Offset of the information element (IE) data blob.
			/// </summary>
			internal uint ieOffset;
			/// <summary>
			/// Size of the IE data blob, in bytes.
			/// </summary>
			internal uint ieSize;
			#endregion
			[JsonProperty]
			public string SSID{
				get{
					return Encoding.UTF8.GetString(this.dot11Ssid.SSID.Take((int)this.dot11Ssid.SSIDLength).ToArray());
				}
				set{
					byte[] Raw = Encoding.UTF8.GetBytes(value);
					this.dot11Ssid.SSID = Raw;
					this.dot11Ssid.SSIDLength = (uint)Raw.Length;
				}
			}
			
			public MacAddress MacAddress{
				get{
					return MacAddress.Parse(this.dot11Bssid);
				}
				set{
					this.dot11Bssid = value.RawData;
				}
			}
			...
			private string MacAddressString{
				...
				set{
					this.dot11Bssid = MacAddress.Parse(value).RawData;
				}
			}
			...
			public override bool Equals(object obj){
				WlanBssEntry k = (WlanBssEntry)obj;
				if(k == null)return false;
				return MacAddressString == k.MacAddressString;
			}
			
			public override string ToString(){
				return JsonConverter.Serialize(this).ToString();
			}
		}
